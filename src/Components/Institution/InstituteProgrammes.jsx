...
export default function InstituteProgrammes({ institutionId }) {
    ...
    const fetchProgrammes = async () => {
      ...
      try {
        ...
        if (progRes.ok) {
          ...
          // Filter programmes belonging to the current institution
          const filtered = allProgrammes.filter(p => {
            const pInstId = typeof p.institutionId === 'object' ? p.institutionId?._id : p.institutionId;
            return pInstId === institutionId;
          });
        ...
        console.error("Error fetching institute programmes:", error);
        setProgrammesList([]);
      } finally {
        setLoading(false);
      }
